...
from loadmodel import initModel, loadPretrainedModel, loadCheckpointModel, loadEvaluateModel
...
def latCodeOptimization(args, filename):
    ...
    for index,fname in indexfname_dict.items():
        if(os.path.exists(os.path.join(args.latcode_folder, 'test_latent_'+args.latfname+'_'+fname+'.py'))):
            ...
            loss, test_latent[fname] = getlatCode(index, test_dataset, test_latent[fname], model, latoptimizer, latscheduler, args, 'lat_'+fname, 'lat_'+fname)
            torch.save(test_latent[fname], os.path.join(args.latcode_folder, 'test_latent_'+args.latfname+'_'+fname+'.py'))
            getDiscAndImpCurvatureOfSurface(grid_uniformsamples, test_latent[fname], model, -1, 300000, args, 'bestlatcode_'+fname)

    #return samples
def latCodeandReg0ModelOptimization(args,filename, hess_delta, add_name=""):
    model = initModel(args)
    cudnn.benchmark = True
    ...
    print("loaded pretrained model")
    test_dataset = initDeepsdfTestDataSet(args, filename)
    grid_uniformsamplepoints = gridData(args=args)
    indexfname_dict = test_dataset.getindexfname()
    args.resamp = 4
    ...
    for index, fname in indexfname_dict.items():
        ...
        if not os.path.exists(baselatpath):
            print("base lat does not exists")
            latent = latCodeOptimization(args, fname)
        else:
            print("base lat loaded")
            latent = torch.load(baselatpath)
        latent = torch.load(baselatpath)
        #model, latent, best_loss = loadCheckpointModelHelper(model, hess_delta+'_'+fname, args.checkpoint_folder)
        test_latent[fname] = torch.ones(args.lat).normal_(mean=0, std=0.01).to(device)
        test_latent[fname].data = latent.data.clone()
        test_latent[fname].to(device)
        test_latent[fname].requires_grad = True
        ...
        if args.losstype == 'dataeikonal':
            model, latent = getlatCodeAndModel(index, test_dataset, grid_uniformsamplepoints, test_latent[fname],  model, latcodeandmodeloptimizer , scheduler, args, 'reg'+str(hess_delta)+'_dataeikonal_'+fname, 'reg'+str(hess_delta)+'_'+add_name+'_'+fname)
        else:
            model, latent = getlatCodeAndModel(index, test_dataset, grid_uniformsamplepoints, test_latent[fname],  model, latcodeandmodeloptimizer , scheduler, args, 'reg'+str(hess_delta)+'_'+fname, 'reg'+str(hess_delta)+'_'+add_name+'_'+fname)
        model.to(device)
        latent.to(device)
        if args.losstype == 'dataeikonal':
            getSurfaceSamplePoints(grid_uniformsamplepoints, latent, model, -1, 300000, args, 'bestmodelreg'+str(hess_delta)+'_dataeikonal_'+fname)
        else:
            getSurfaceSamplePoints(grid_uniformsamplepoints, latent, model, -1, 300000, args, 'bestmodelreg'+str(hess_delta)+'_'+fname)

def latCodeandModelOptimization(args,filename, hess_delta, add_name=""):
    ...
    test_latent = {}
    for index, fname in indexfname_dict.items():
        if not ((os.path.exists(os.path.join(args.checkpoint_folder, 'model_best_reg0.0_'+fname+'.pth.tar'))) or (os.path.exists(os.path.join(args.checkpoint_folder, 'model_best_reg0_'+fname+'.pth.tar')))): 
            args.resamp = 4
            latCodeandReg0ModelOptimization(args, filename, '0')
            args.resamp = 2
            model, latent = loadEvaluateModel(model, 'reg0_'+fname, args)
            mcube_points = getSurfaceSamplePoints(grid_uniformsamplepoints, latent, model, -1, 10000, args, 'bestmodelreg0_'+fname)
        elif os.path.exists(os.path.join(args.checkpoint_folder, 'model_best_reg0_'+fname+'.pth.tar')):
            model, latent = loadEvaluateModel(model, 'reg0_'+fname, args)
            mcube_points = getSurfaceSamplePoints(grid_uniformsamplepoints, latent, model, -1, 10000, args, 'bestmodelreg0_'+fname)

        if model is None:
            return
        if mcube_points is None:
            print(fname+" IF level is 0")
            model = initModel(args)
        model.to(device)
        model.eval()
        test_latent[fname] = torch.ones(args.lat).normal_(mean=0, std=0.01).to(device)
        if mcube_points is not None:
            test_latent[fname].data = latent.data.clone()
        test_latent[fname].to(device)
        test_latent[fname].requires_grad = True
        print("test_latent=", test_latent[fname][0:10], flush=True)
        ...
        getlatCodeAndModel(index, test_dataset, grid_uniformsamplepoints, test_latent[fname],  model, latcodeandmodeloptimizer , scheduler, args, args.losstype+'_reg'+str(hess_delta)+'_'+add_name+'_'+fname, args.losstype+'_reg'+str(hess_delta)+'_'+add_name+'_'+fname)

def latCodeandHessOptimization(args, filename, hess_delta, add_name=""):
    model = initModel(args)
    cudnn.benchmark = True
    ...
    for index, fname in indexfname_dict.items():
        ...
        latoptimizer = initlatentOptimizer(test_latent[fname], args)
        latscheduler = ReduceLROnPlateau(latoptimizer, factor=0.9,patience=15,mode='min',threshold=1e-4, eps=0, min_lr=0)
        getlatCodeAndModel(index, test_dataset, grid_uniformsamplepoints, test_latent[fname],  model, latoptimizer , latscheduler, args, args.losstype+'_reg'+str(hess_delta)+'_fromlat_'+add_name+'_'+fname, args.losstype+'_reg'+str(hess_delta)+'_fromlat_'+add_name+'_'+fname)

def latCodeandModelOptimizationWithBaselat(args,filename, hess_delta):
    model = initModel(args)
    cudnn.benchmark = True
    ...
    for index, fname in indexfname_dict.items():
        ...
        getlatCodeAndModel(index, test_dataset, grid_uniformsamplepoints, test_latent[fname],  model, latcodeandmodeloptimizer , scheduler, args, 'fromlatreg'+str(hess_delta)+'_'+fname, 'fromlatreg'+str(hess_delta)+'_'+fname)

def getlatCode(index, test_dataset, latent, model, optimizer, scheduler, args, modelname='lat', reconstructname='bestmodellat'):
    ...
    #for epoch in range(500):
        ...
        isbestloss = epochloss < best_loss
        #if isbestloss:
        ...
        #        isbestloss = False
        #        print("IF level not in 0", flush=True)
...
def getlatCodeAndModel(index, test_dataset, grid_uniformsamplepoints, latent, model, optimizer, scheduler, args, modelname='paramlat', reconstructname='bestparamlat', best_loss=2e10):
    ...
    #if args.resamp == 4:
    #    numiter = 2000
    if args.hess_delta <= 0 or args.resamp == 4 :
        numiter = 1000
    curr_lr = args.lr
    for param_group in optimizer.param_groups: 
        print("LR step :: ",param_group['lr']) 
    ...
    #for epoch in range(5000):
        ...
        for chunk in range(550):
            ...
            if args.losstype == 'dataeikonal':
                gradient = getGradient(predicted_sdf, sampled_points)
                sdfloss = datafidelity_testloss(predicted_sdf, gt_sdf_tensor, latent, epoch,  args, gradient)
            else:
           ...
           #     printinterval = 1
            ...
            if args.hess_delta > 0 and args.resamp == 2:
...
#        if args.resamp == 1 and epoch % 10 == 0:
#            newmcube_points = getSurfaceSamplePoints(grid_uniformsamplepoints, latent, model, epoch, 300000, args, reconstructname)
#
#            if newmcube_points is not None:
#                mcube_points = newmcube_points.copy()
#        elif args.resamp == 3 and isbestloss:
#            newmcube_points = getSurfaceSamplePoints(grid_uniformsamplepoints, latent, model, epoch, 300000, args, reconstructname)
#
#            if newmcube_points is not None:
#                mcube_points = newmcube_points.copy()
        if args.resamp == 2 and args.hess_delta > 0:
            newmcube_points = getSurfaceSamplePoints(grid_uniformsamplepoints, latent, model, epoch, 250000, args, reconstructname)
        ...
        scheduler.step(epochloss) 
    if epoch == 20:
        save_curr_checkpoint({"epoch": epoch , "lr": curr_lr, "state_dict": model.state_dict(), "best_loss": best_loss, "optimizer": optimizer.state_dict(), "test_latent":latent}, checkpoint_folder=args.checkpoint_folder,modelname=modelname+'_epoch20')
    save_curr_checkpoint({"epoch": epoch , "lr": curr_lr, "state_dict": model.state_dict(), "best_loss": best_loss, "optimizer": optimizer.state_dict(), "test_latent":latent}, checkpoint_folder=args.checkpoint_folder,modelname=modelname)
          
    return best_model, best_latent 

def getlatCodeAndModelWithCurvature(index, test_dataset, grid_uniformsamplepoints, latent, model, optimizer, scheduler, args, modelname='model_best_modellat', reconstructname='bestmodellatcode'):
    #for i in range(num_batch)::w!
