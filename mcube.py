...
def getmcubePoints(dataset, latent, model, args):
    ...
    for i in range(num_batch):
        ...
        with torch.no_grad():
            #latvec = latent(torch.tensor(0)).expand(this_bs,-1).to(device)
            latvec = latent.expand(this_bs,-1).to(device)
            pred_sdf_tensor = model(torch.cat([latvec,xyz_tensor],dim=-1))
            if args.clamp:
    ...
    mid = (IF.max() + IF.min())/2
    #print("mid = ",mid)
    if args.isolevel == 0 and ((IF.min() >= 0 or IF.max() <= 0 or np.isnan(np.array(IF.min())) or np.isnan(IF.max()))):
    ...
    if args.isolevel == 0:
        ...
        verts, simplices, normals, values = measure.marching_cubes(IF) #,0) #,0) #,mid)
